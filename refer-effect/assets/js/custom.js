...
function agentCheck(){
    ...
    const browserList = {
        mac: agent.match(/macintosh/i), //i: 소문자 대문자 구별
    ...
    for( prop in browserList ){ //browserList에 있는 속성 값만 가지고 옴
        if (browserList[prop]){
            //console.log(prop)
            document.querySelector("body").classList.add(prop);
            document.getElementsByTagName("body").classList.add(prop);  //익스플로어10 아래
        }
    }
}
