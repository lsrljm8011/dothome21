...
<?php
    ...
    // var_dump(filter_var('serin@naver.com', FILTER_VALIDATE_EMAIL));
    ...
    if( $check_number == false ){
        msg("휴대폰 번호가 정확하지 않습니다. <br> 올바른 휴대폰 번호(000-0000-0000)를 적어주세요!");
        exit;
    }

    //데이터 가져오기 -> 유효성 검사 -> 중복 검사(이메일, 핸드폰 번호) -> 회원가입
    ...
    if($isEmailCheck = true && $isPhoneCheck = true){
        ...
        if($result){
            msg("회원가입을 축하합니다. <br> 로그인 해주세요!");
        } else {
            msg("에러발생03 - 관리자에게 문의하세요!");
            exit;
        }
    } else {
        msg("이메일 또는 핸드폰 번호가 틀립니다. <br> 다시 한번 확인해주세요!");
        exit;
    }
?>
                </div>
